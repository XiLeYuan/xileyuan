...
object MMKVManager {
     ...
     * 存储对象（需要实现Parcelable）
     */
    fun <T : Parcelable> putParcelable(key: String, value: T, mmkv: MMKV = defaultMMKV) {
        mmkv.encode(key, value)
    }
    ...
    fun <T : Parcelable> getParcelable(key: String, clazz: Class<T>, mmkv: MMKV = defaultMMKV): T? {
